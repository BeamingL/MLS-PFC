...
class PEGFSSampler:
    """Adaptive replay sampler with a parameterized memory curve."""
    ...
    def init_for_new_task(self, memory_buffer: MemoryBuffer, historical_params: Dict[int, torch.Tensor]):
        """Initialize task iterators and initial probabilities before training a new task."""
    ...
    def maybe_update_probabilities(self, step: int, current_params: torch.Tensor):
        """Update replay probabilities every U steps."""
        if not self._task_ids:
            return
    ...
    def sample_replay_batch(self, batch_size: int) -> List[dict]:
        """Sample a replay batch according to the current probability distribution."""
    ...
    def get_current_probabilities(self) -> Dict[int, float]:
        """Return current sampling probabilities."""
